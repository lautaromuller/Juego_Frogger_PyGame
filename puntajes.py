...
def agregar_jugador(jugador):
    with sqlite3.connect("bd_puntos.db") as conexion:
        try:
            conexion.execute("insert into puntajes(nombre,puntos,tiempo) values (?,?,?)", (jugador.nombre, jugador.puntos,jugador.tiempo))
...
def eliminar_todos_los_jugadores():
    with sqlite3.connect("bd_puntos.db") as conexion:
        try:
            ...
            print("Todos los datos han sido eliminados de la tabla puntajes")
