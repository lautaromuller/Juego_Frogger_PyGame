...
def nivel_2(nombre,puntos,tiempo_total):
    ...
    pygame.time.set_timer(tiempo_objeto,33)
    ...
    l_troncos_der = troncos.crear_lista_troncos_der(36)
    l_troncos_izq = troncos.crear_lista_troncos_izq(36)
    l_autos_der = autos.crear_lista_autos_der(10,12000)
    l_autos_izq = autos.crear_lista_autos_izq(10,12800)
    ...
    while ejecutar:
        ...
        for evento in lista_eventos:
            ...
            if evento.type == pygame.USEREVENT:
                if evento.type == tiempo_objeto:
                    autos.mover_auto_der(l_autos_der,12000)
                    autos.mover_auto_izq(l_autos_izq,12800)
            ...
            if evento.type == pygame.USEREVENT+1:
                if evento.type == tiempo_bicho:
                    insecto.mover(bicho)
                    #print(f"Nombre: {nombre} Tiempo:{tiempo_total} Puntos: {puntos}")
        ...
        if termino:
            ...
            puntos += 300
            punto = fuente_punto.render(f"Score: {puntos}",True,(0,0,0))
            nivel_3.nivel_3(nombre,int(puntos),tiempo_total)
            ejecutar = False
